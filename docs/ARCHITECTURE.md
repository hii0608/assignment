...
판단, **의존성 없는 단일 노드 구성**을 선택했다. 작업 분배는 브로커 push가 아닌
**저장소 폴링(DB-as-queue) 패턴**으로 구현한다.

**이 구성의 핵심 기술 과제**: API 핸들러와 스케줄러라는 두 컴포넌트가
...
("대기 → 실행 → 성공/실패 + 재시도 루프 + 사용자 취소")을 채택한 것.
...
- **processing 중인 job의 취소/수정**: 건당 처리 시간이 수 초로 짧아, 취소 요청이
  ...
  실행됨"이라는 거짓 계약이 되므로 지원하지 않는 것이 정직한 설계다. 건당 처리가
  길어지는 도메인이 연결되면 cancelRequested 플래그(cooperative cancellation)
  패턴으로 전환한다. (§10)
- **completed/cancelled에서의 모든 변경**: 종착 상태는 불변 기록.
- **사용자가 status를 processing으로 직접 변경**: claiming은 워커 전용 연산.

### attempts 리셋 근거

수동 재시도는 "조건이 달라진 새 출발"로 간주한다 (전형 사례: 외부 의존성 장애로
...
필드로 추가한다 — 현재 요구에 없으므로 제외하고, 이력 추적은 로그 계층이 담당(§8).
...
- title/description 수정 허용 상태: **{pending, failed}**.
  failed에서의 수정을 허용하는 이유: 잘못된 내용 때문에 실패했을 수 있으므로
  "고치고 재시도"는 자연스러운 운영 워크플로우이며, PATCH 한 번으로 가능해진다:
...
RDBMS였다면 트랜잭션/행 락이 담당할 일을 애플리케이션 레벨에서 수행하는 것.
파일 기반 저장을 선택한 대가이며, 이 규모에서는 수용 가능한 비용이다.
...
- **처리량**: 전체 직렬화의 처리량 상한은 파일 쓰기 ~1ms 기준 이 규모에서 병목
  아님. 읽기 비중이 커지면 read-write lock으로 개선 가능.
- logs.txt는 append-only라 뮤텍스 불필요 (O_APPEND + 단일 write stream으로 충분).
  손실 가능한 상태 데이터(뮤텍스 보호)와 append-only 로그는 접근 패턴이 다르므로
  보호 수준도 다르게 가져간다.
...
**설계 원칙**: `배치 크기 × 건당 최대 시간 < 주기`를 유지해 정상 상황에서 overlap이
발생하지 않게 하고, overlap 가드는 안전망으로만 동작시킨다. 시스템의 정상 동작이
안전망에 의존해서는 안 된다.
...
1. **건당 처리가 길어지면**: cancelRequested 기반 cooperative cancellation 도입
2. **읽기 트래픽 증가**: read-write lock 전환, ETag/If-Match 낙관적 동시성 제어
