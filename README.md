...
- Node.js 20 이상 권장 (Node v22, v24에서 테스트 통과 확인)
...
- 저장소에 포함된 `jobs.json` 시드에 각 상태별 job이 1건씩 들어 있다.
  ...
  processing 시드는 부팅 시 크래시 복구(startup sweep)로 pending에 복귀한 뒤
  처리되므로, 서버를 켜면 상태가 바뀌는 것이 정상 동작이다.
...
목록/검색 응답은 결과(`data`)와 페이지 정보(`meta`)를 담은 구조다 (200, `?page=1&limit=2` 실측):
...
API 설계·데이터 처리·성능에 대해 의도적으로 결정한 것들, 고민했던 지점,
되돌린 결정, 남은 과제를 기록한다. 상세 근거와 대안 비교는
[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), 특히 §9 결정 기록 참조.
...
이중 방어를 둔다. 배치 내 처리는 순차다 — 제약식이 순차 기준 최악치로 계산되어
있다. 부팅 시에는 processing으로 남은 고아 job을 전부 pending으로 되돌린다
(크래시 복구, startup sweep). 단일 프로세스 전제에서 부팅 시점에 processing인
job은 그것을 처리하던 워커가 살아 있을 수 없으므로 전부 고아이다.
처리 핸들러는 인터페이스로 분리된 스텁이다. 실제 도메인 작업을 연결할 때
워커 코드 변경 없이 교체할 수 있다.
...
- **크래시 복구는 startup sweep** — 부팅 시 processing 고아 job을 일괄 pending
  복귀 (attempts 불변, scheduler.recovery 로그 기록). 타임아웃 reaper는 단일
  프로세스에선 복잡도만 증가해 기각. 한계: 반복 크래시를 유발하는 job은 무한
  재시도됨(poison pill)
- **배치 내 처리는 순차 유지** — Promise.all 동시 처리는 검토 후 기각. 완료
  지연을 지배하는 건 폴링 주기(30초)라 실익이 없고, 실제 핸들러(외부 API) 연결 시
  동시 호출은 rate limit 위험이 있어 순차가 보수적 기본값. 동시 처리가 필요해지면
  배치 크기와 분리된 concurrency 설정으로 도입
